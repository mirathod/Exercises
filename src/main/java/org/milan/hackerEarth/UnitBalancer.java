...
public class UnitBalancer {
	...
	private static String formatOutput(List<String> equations, List<String> sortedUnits) {
		...
		for (int i = 0; i < sortedUnits.size() - 1;) {
			...
			String temp = equations.stream().filter(s -> (s.contains(unitOne) && s.contains(unitTwo))).findFirst()
					.get();
...
class ConversionMap {
	...
	/**
	 * Adds a conversion rule by mapping a unit to another unit Example:
	 * addConversion("meters", 1000, "kilometers", 1)
	 * 
	 * @param unit1
	 *            The first unit
	 * @param amount1
	 *            The amount of the first unit
	 * @param unit2
	 *            The second unit
	 * @param amount2
	 *            The amount of the second unit
	 */
	public void addConversion(String unit1, double amount1, String unit2, double amount2) {
		...
		Set<UnitConversion> conversionsUnit1 = conversionMap.get(unit1), conversionsUnit2 = conversionMap.get(unit2);
	 ...
	 *            The first unit
	 * @param unit2
	 *            The second unit (destination unit)
	 * @param visited
	 *            A map consisting of the shortest known paths to each unit we've
	 *            visited so far
	 * @return One of the shortest conversion paths that can be used to convert
	 *         between units or null if no such path can be found
	 */
	public ConversionPath getConversionPath(String unit1, String unit2, Map<String, ConversionPath> visited) {
		...
		// If unit1 and unit2 are the same, then we've reached the destination, return
		// the path so far
		...
		for (UnitConversion conversion : possibleConversions) {
			...
			// If going to the unit through this unit is shorter than what was previously
			// there
			// then search for the destination from that unit
			if (path == null || path.pathLength() > currentPath.pathLength() + 1) {
				// Update the path on the other unit, because there is now a shorter way to get
				// there
	 ...
	 * @return A ConversionPath representing the shortest path to convert from unit1
	 *         to unit2
	 */
	...
	public List<String> getResult() {
		List<String> units = conversionMap.keySet().stream().filter(s -> conversionMap.get(s).size() == 1)
...
/**
 * A wrapper for String[], an array of units. The path is immutable. Example: to
 * convert from seconds to hours the ConversionPath is ["minutes", "hours"]
 */
class ConversionPath {
	 ...
	 * 
	 * @param units
	 *            The units that this conversion path has
	 */
